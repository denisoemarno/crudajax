...
$nip = $_POST['nip']; // Ambil data nis dan masukkan ke variabel nis
...
if(isset($_POST['ubah_foto'])){ // Jika user menceklis checkbox yang ada di form ubah, lakukan :
  ...
  if(move_uploaded_file($tmp, $path)){ // Jika proses upload sukses
    ...
    if(is_file("../fotoguru/".$data['foto'])) // Jika foto ada
      unlink("../fotoguru/".$data['foto']); // Hapus file foto sebelumnya yang ada di folder foto
    
    // Proses ubah ke Database
    $sql = $pdo->prepare("UPDATE guru SET nama=:nama, jenis_kelamin=:jk, telp=:telp, alamat=:alamat, foto=:foto WHERE nip=:nip");
  ...
  // Proses ubah ke Database
  $sql = $pdo->prepare("UPDATE guru SET nama=:nama, jenis_kelamin=:jk, telp=:telp, alamat=:alamat WHERE nip=:nip");
  $sql->bindParam(':nama', $nama);
  $sql->bindParam(':jk', $jenis_kelamin);
  $sql->bindParam(':telp', $telp);
  $sql->bindParam(':alamat', $alamat);
